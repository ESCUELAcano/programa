...
#include <fstream>
#include "ConsultarAlumno.h"

#include <iostream>
using namespace std;

struct consultaAlumno ConsultarAlumno(string CURP) {


    /*VARIABLES*/
    struct consultaAlumno a;
    MYSQL* coneccion;
    ifstream archivoSQL{ "SCRIPTS/llamarConsultarAlumno.sql" };
    ...
    //REMPLAZAR CURP
    pos = ejecutarScript.find("CURPAlumno", 0);
    if (pos < string::npos) {
        ejecutarScript.replace(pos, string("CURPAlumno").length(), "\'" + CURP + "\'");
    }
    //TERMINA CURP
